...
import { parseFile, parseCollection, clearParserCache, removeFileFromCache } from './parser';
...
describe('Parser (Shared)', () => {
  ...
  it('parseFile should parse a valid bru file', async () => {
    mockFs.existsSync.mockImplementation(() => true);
    mockFsPromises.readFile.mockImplementation(async () => `meta {
  name: Get User
  type: http
}
get {
  ...
  it('parseFile should return null if parsing fails', async () => {
    mockFs.existsSync.mockImplementation(() => true);
    mockFsPromises.readFile.mockImplementation(async () => { throw new Error('File error'); });
    const req = await parseFile('/tmp', '/tmp/file.bru');
    expect(req).toBeNull();
  });
  ...
  it('parseCollection should parse folders and requests correctly', async () => {
    ...
    mockFsPromises.readdir.mockImplementation(async (path: string) => {
      if (path === '/tmp') return [{ name: 'folder1', isDirectory: () => true, isFile: () => false }, { name: 'req1.bru', isDirectory: () => false, isFile: () => true }, { name: 'environments', isDirectory: () => true, isFile: () => false }];
      if (path === '/tmp/folder1') return [{ name: 'folder.yml', isDirectory: () => false, isFile: () => true }, { name: 'req2.bru', isDirectory: () => false, isFile: () => true }];
      if (path === '/tmp/environments') return [{ name: 'dev.yml', isDirectory: () => false, isFile: () => true }];
      return [];
    });
    mockFsPromises.readFile.mockImplementation(async (path: string) => {
      if (path.endsWith('req1.bru') || path.endsWith('req2.bru')) return `meta {\n  name: Req\n  type: http\n}\nget {\n  url: /\n}`;
      if (path.endsWith('folder.yml')) return `info:\n  name: Folder 1`;
      if (path.endsWith('dev.yml')) return `name: Dev\nvariables:\n  - name: url\n    value: localhost`;
      return '';
    });
