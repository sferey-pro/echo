...
export async function parseFile(
  ...
  try {
    ...
    if (parsed?.info?.type === "http") {
      ...
      const folderPath = relativeFilePath.substring(
        0,
        relativeFilePath.lastIndexOf("/"),
      );
      const folderId = folderPath
        ? `f-${Buffer.from(folderPath).toString("base64url")}`
        : "root";
...
export async function parseCollection(
  ...
  async function traverse(currentPath: string): Promise<BrunoFolder | null> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        entry.isFile() &&
        (entry.name.endsWith(".yml") || entry.name.endsWith(".bru"))
      ) {
  ...
  try {
    ...
    for (const entry of rootEntries) {
      if (
        entry.isDirectory() &&
        !entry.name.startsWith(".") &&
        entry.name.toLowerCase() !== "environments"
      ) {
        ...
        if (entry.isFile()) {
          ...
          try {
            if (entry.name.endsWith(".yml") || entry.name.endsWith(".json")) {
              const parsed = entry.name.endsWith(".yml")
                ? parseYaml(content)
              ...
              const rawName =
                ...
                basename(
                  entry.name,
                  entry.name.endsWith(".yml") ? ".yml" : ".json",
                );
              const name = parentRelPath
                ? `${parentRelPath}/${rawName}`
                : rawName;
  ...
  async function searchAndLoadEnvironments(
    currentPath: string,
    relPath: string = "",
  ) {
      ...
      for (const entry of entries) {
        if (
          ...
          if (entry.name.toLowerCase() === "environments") {
            await loadEnvironmentsFromDir(
              join(currentPath, entry.name),
              relPath,
            );
          } else {
            await searchAndLoadEnvironments(
              join(currentPath, entry.name),
