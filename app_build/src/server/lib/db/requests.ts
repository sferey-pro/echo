...
export const getRequestMeta = (): Record<string, { isStarred: boolean }> => {
  ...
  const query = db.query("SELECT * FROM request_meta WHERE collection_name = $col");
  const results = query.all({ $col: collectionName }) as { request_id: string; is_starred: number }[];
...
export const updateRequestMeta = (requestId: string, isStarred: boolean) => {
 ...
 `);
  query.run({ $id: requestId, $col: collectionName, $isStarred: isStarred ? 1 : 0 });
};

export const clearBrunoTables = () => {
  const collectionName = getActiveCollection();
  db.query("DELETE FROM bruno_requests WHERE collection_name = $col").run({ $col: collectionName });
  db.query("DELETE FROM bruno_folders WHERE collection_name = $col").run({ $col: collectionName });
  db.query("DELETE FROM bruno_environments WHERE collection_name = $col").run({ $col: collectionName });
};
...
export const syncBrunoItemsToDb = (
  ...
  const transaction = db.transaction(() => {
    db.query("UPDATE bruno_requests SET is_obsolete = 1 WHERE collection_name = $col").run({ $col: collectionName });
    db.query("UPDATE bruno_folders SET is_obsolete = 1 WHERE collection_name = $col").run({ $col: collectionName });
    db.query("UPDATE bruno_environments SET is_obsolete = 1 WHERE collection_name = $col").run({ $col: collectionName });
    ...
    for (const r of requests)
      insertReq.run({ $id: r.id, $col: collectionName, $data: JSON.stringify(r) });
    ...
    for (const f of folders)
      insertFolder.run({ $id: f.id, $col: collectionName, $data: JSON.stringify(f) });
    ...
    for (const e of environments)
      insertEnv.run({ $name: e.name, $col: collectionName, $data: JSON.stringify(e) });
  });
  transaction();
};
...
export const getCollectionFromDb = () => {
  const collectionName = getActiveCollection();
  const reqRows = db.query("SELECT * FROM bruno_requests WHERE collection_name = $col").all({ $col: collectionName }) as {
    id: string;
    data: string;
    is_obsolete: number;
  }[];
  const folderRows = db.query("SELECT * FROM bruno_folders WHERE collection_name = $col").all({ $col: collectionName }) as {
    id: string;
    data: string;
    is_obsolete: number;
  }[];
  const envRows = db.query("SELECT * FROM bruno_environments WHERE collection_name = $col").all({ $col: collectionName }) as {
    name: string;
    data: string;
    is_obsolete: number;
  }[];
