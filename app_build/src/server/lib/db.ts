...
export const applyScenarioActions = (actions: ScenarioAction[]) => {
  ...
  for (const action of actions) {
    ...
    db.query(
      "UPDATE mock_variants SET is_mocked = 0 WHERE request_id = $id",
    ).run({ $id: action.requestId });

    // Upsert a "Scenario" variant
    const checkQuery = db.query(
      "SELECT id FROM mock_variants WHERE request_id = $id AND name = 'Scenario' LIMIT 1",
    );
    const exists = checkQuery.get({ $id: action.requestId }) as {
      id: string;
    } | null;
 ...
 `);
      updateQuery.run({
        ...
        $selectedExample:
          action.selectedExample !== undefined ? action.selectedExample : null,
        $pathParamsOverrides: action.pathParamsOverrides
          ? JSON.stringify(action.pathParamsOverrides)
          : null,
      });
    } else {
...
export const syncBrunoItemsToDb = (
  ...
  const transaction = db.transaction(() => {
    ...
    const insertReq = db.query(
      `INSERT INTO bruno_requests (id, data, is_obsolete) VALUES ($id, $data, 0) ON CONFLICT(id) DO UPDATE SET data = excluded.data, is_obsolete = 0`,
    );
    for (const r of requests)
      insertReq.run({ $id: r.id, $data: JSON.stringify(r) });

    const insertFolder = db.query(
      `INSERT INTO bruno_folders (id, data, is_obsolete) VALUES ($id, $data, 0) ON CONFLICT(id) DO UPDATE SET data = excluded.data, is_obsolete = 0`,
    );
    for (const f of folders)
      insertFolder.run({ $id: f.id, $data: JSON.stringify(f) });

    const insertEnv = db.query(
      `INSERT INTO bruno_environments (name, data, is_obsolete) VALUES ($name, $data, 0) ON CONFLICT(name) DO UPDATE SET data = excluded.data, is_obsolete = 0`,
    );
    for (const e of environments)
      insertEnv.run({ $name: e.name, $data: JSON.stringify(e) });
  });
  transaction();
};

export const getCollectionFromDb = () => {
  const reqRows = db.query("SELECT * FROM bruno_requests").all() as {
    id: string;
    data: string;
    is_obsolete: number;
  }[];
  const folderRows = db.query("SELECT * FROM bruno_folders").all() as {
    id: string;
    data: string;
    is_obsolete: number;
  }[];
  const envRows = db.query("SELECT * FROM bruno_environments").all() as {
    name: string;
    data: string;
    is_obsolete: number;
  }[];
...
export const cleanupObsoleteItems = () => {
  ...
  db.exec(
    "DELETE FROM mock_variants WHERE request_id NOT IN (SELECT id FROM bruno_requests)",
  );
  db.exec(
    "DELETE FROM request_meta WHERE request_id NOT IN (SELECT id FROM bruno_requests)",
  );
};
