...
export async function handleScenariosRoute(req: Request, url: URL): Promise<Response | null> {
 ...
 if (!body.name) return new Response("Bad Request: missing name", { status: 400, headers: {  } });
 ...
 try {
 const body = await req.json();
 if (!body.id) return new Response("Bad Request: missing id", { status: 400, headers: {  } });
 
 const scenarios = getScenarios();
 const scenario = scenarios.find(s => s.id === body.id);
 if (!scenario) return new Response("Scenario not found", { status: 404, headers: {  } });
