...
describe('API Route: /api/scenarios', () => {
  ...
  it('should list scenarios', async () => {
    const req = new Request('http://localhost/api/scenarios', { method: 'GET' });
  ...
  it('should create a scenario automatically collecting current mock states if no actions provided', async () => {
    ...
    mockVariants.set('req1', [{ id: 'var1', isMocked: true, payload: 'ok', statusCode: 200, latencyMs: 0, selectedExample: null, pathParamsOverrides: null }]);
  ...
  it('should update a scenario', async () => {
    ...
    const createRes = await handleScenariosRoute(createReq, new URL(createReq.url));
    ...
    const updateRes = await handleScenariosRoute(updateReq, new URL(updateReq.url));
  ...
  it('should apply a scenario', async () => {
    ...
    const applyRes = await handleScenariosRoute(applyReq, new URL(applyReq.url));
