...
export function EnvironmentViewerModal({ isOpen, onClose, environments, activeEnvironmentName }: EnvironmentViewerModalProps) {
 ...
 <div className="bg-background border-2 border- rounded-xl shadow-[8px_8px_0px_black] w-full max-w-2xl flex flex-col max-h-[85vh]">
 ...
 <div className="p-8 border-2 border-dashed border- rounded-xl text-center flex flex-col items-center">
 ...
 <div className="bg-white border-2 border- rounded-lg overflow-hidden">
 <div className="bg- p-3 border-b-2 border-">
 <h3 className="font-black text-black">Variables de l'environnement : {activeEnv.name}</h3>
 </div>
 <div className="overflow-x-auto">
 <table className="w-full text-left text-sm font-bold">
 <thead className="bg-slate-100 border-b-2 border-">
 <tr>
 <th className="p-3 border-r-2 border- w-1/3 text-foreground">Nom</th>
 <th className="p-3 text-foreground">Valeur</th>
 </tr>
 </thead>
 <tbody>
 {activeEnv.variables.map((v, i) => (
 <tr key={i} className="border-b-2 border- last:border-b-0 hover:bg-slate-50 :bg-slate-800">
 <td className="p-3 border-r-2 border- font-mono text-xs text-foreground">
 {v.name}
 </td>
 ...
 <div className="p-4 border-t-2 border- flex justify-end">
 <button 
 onClick={onClose}
 className=" bg-slate-200 text-black font-black px-6 py-2"
 >
 Fermer
 </button>
 </div>
