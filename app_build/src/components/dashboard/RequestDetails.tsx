...
export function RequestDetails() {
 ...
 <div className="p-4 bg-card border-b border-border z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
 <div className="w-full">
 <h2 className="text-xl font-black text-foreground tracking-tight flex items-center gap-2">
 <button 
 onClick={handleToggleStar}
 disabled={isSaving}
 className={`text-2xl hover:scale-110 transition-transform ${request.isStarred ? 'text-' : 'text-slate-400'}`}
 ...
 className="w-full accent- h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer border-2 border-"
 />
 </div>
 </div>
 ...
 <SelectContent className="border-2 border- shadow-[4px_4px_0px_black] font-bold">
 ...
 <div className={`flex-1 flex flex-col rounded-xl overflow-hidden relative ${isPayloadModified ? '' : 'border-[3px] border-'}`}>
 {isPayloadModified && (
 <div className="absolute top-2 right-6 z-20 text- font-black text-sm pointer-events-none drop-shadow-md">
 ...
 <button 
 className=" bg- text-black w-full sm:w-auto px-6 py-3 flex items-center justify-center gap-2"
 onClick={() => {
 ...
 ÉTAT ACTUEL : 
 <span className={` text-black text-sm px-3 py-1 ${isPayloadModified ? 'bg-' : 'bg-slate-200'}`}>
