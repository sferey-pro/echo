...
import type { BrunoFolder, ApiRequest, BrunoEnvironment } from '../../shared/lib/parser';
...
interface AppState {
 ...
 updateLocalVariant: (requestId: string, variantId: string, updates: Partial<MockVariantDef>) => void;
...
export const useStore = create<AppState>((set, get) => ({
 ...
 updateLocalVariant: (requestId, variantId, updates) => set((state) => ({
    requests: state.requests.map((r) => {
      ...
      const newVariants = (r.variants || []).map((v) => {
        ...
        if (updates.isMocked !== undefined && updates.isMocked && v.id !== variantId) {
 ...
 set({ isLoading: false, isError: true, errorMessage: error instanceof Error ? error.message : "Erreur inconnue" });
 }
 }
}));
