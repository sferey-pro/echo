...
export function RequestList({
  ...
  const renderVirtualItem = (virtualItem: VirtualItem) => {
    ...
    if (item.type === "starred-header") {
      return (
        ...
        >
          <CaretRight
            className={cn(
              "w-3.5 h-3.5 mr-1 text-muted-foreground transition-transform duration-200",
          ...
          <span
            className="truncate"
            style={{ textShadow: "0 0 8px var(--color-)" }}
          >
            Mes Favoris
          </span>
        </div>
      ...
      return (
        ...
        >
          ...
          >
            ...
            {item.isExpanded ? (
              <FolderOpen
                className="w-3.5 h-3.5 mr-1.5 text-yellow-600/80 z-10"
                weight="fill"
              />
            ) : (
              <Folder
            ...
            <span
              className={cn(
                "truncate z-10",
                item.folder.isObsolete && "line-through text-muted-foreground",
              )}
            >
              {item.folder.name}
            </span>
            {item.folder.isObsolete && (
              <span className="ml-2 z-10 text-[9px] uppercase font-bold tracking-wider text-red-500 border border-red-500/30 bg-red-500/10 px-1 py-0.5 rounded">
                Obsolète
              </span>
            )}
          </div>
    ...
    if (item.type === "request" || item.type === "starred-request") {
      ...
      const depth =
        item.type === "starred-request"
          ? 1
          : (item as Extract<ListItem, { depth: number }>).depth;
      ...
      return (
        ...
        >
          ...
          >
            ...
            <span
              className={cn(
                "truncate flex-1 z-10 font-medium",
                req.isObsolete && "line-through text-muted-foreground",
              )}
            >
              {req.isStarred && (
                <Star
                  className="w-3 h-3 inline mr-1 text-yellow-500"
                  weight="fill"
                />
              )}
              {req.isObsolete && (
                <span className="text-[9px] uppercase font-bold tracking-wider text-red-500 border border-red-500/30 bg-red-500/10 px-1 py-0.5 rounded mr-1">
                  Obsolète
                </span>
              )}
              {req.name}
  ...
  return (
    <div className="h-full bg-card/50 flex flex-col font-sans">
      <div className="p-3 bg-transparent flex items-center gap-2">
        <h2 className="text-xs font-black text-foreground tracking-wide uppercase flex-1">
          Explorateur
        </h2>
      ...
      >
        ...
        >
          {rowVirtualizer
            .getVirtualItems()
            .map((virtualItem) => renderVirtualItem(virtualItem))}
        </div>
      </div>
    </div>
  );
