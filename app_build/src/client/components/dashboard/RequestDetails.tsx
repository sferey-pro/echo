...
export function RequestDetails() {
  ...
  useEffect(() => {
    if (request && variants.length > 0) {
      if (!activeVariantId || !variants.find((v) => v.id === activeVariantId)) {
        ...
        const defaultVar =
          variants.find((v) => v.id === `${request.id}-default`) || variants[0];
        if (defaultVar) {
  ...
  const handleToggleMock = async () => {
    ...
    try {
      const updates = {
        isMocked: !activeVariant.isMocked,
        payload,
        statusCode,
        latencyMs,
        pathParamsOverrides,
          ...
          <h2 className="text-xl font-black text-foreground tracking-tight flex items-center gap-2">
            ...
            >
              {request.isStarred ? (
                ...
                <Star weight="regular" className="w-6 h-6" />
              )}
            </Button>
            ...
            {(urlParams.variables.length > 0 ||
              ...
              <div className="flex flex-wrap gap-2 mt-1 items-center">
                ...
                {urlParams.variables.map((param) => (
                  <div
                    key={param}
                    className="flex items-center overflow-hidden h-8 border border-border rounded-md bg-muted"
                    title="Variable d'environnement (Lecture seule)"
                  >
                ...
                <AlertDialogContent className="shadow-lg rounded-xl">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-muted-foreground">
                      Cette action supprimera définitivement cette variante.
            ...
            <SelectContent>
              {variants.map((v: MockVariantDef) => (
                <SelectItem key={v.id} value={v.id}>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${v.isMocked ? "bg-green-500" : "bg-slate-300"}`}
                    />
