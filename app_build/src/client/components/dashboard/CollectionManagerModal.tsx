      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setConfirmDialog({
            ...
            description:
              "Un dossier portant le nom de ce dépôt existe déjà localement. Voulez-vous l'écraser et recommencer le clonage ?",
  ...
  const handleActivate = async (name: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(
          `Erreur lors de l'activation: ${err.error || err.message || "Inconnue"}`,
        );
      }
    } catch (e) {
      toast.error(
      ...
      onConfirm: async () => {
        try {
          ...
          if (res.ok) {
            ...
            const err = await res.json().catch(() => ({}));
            toast.error(
              `Erreur lors de la suppression: ${err.error || err.message || "Inconnue"}`,
            );
          }
        } catch (e) {
          console.error(e);
      ...
      <DialogContent className="sm:max-w-3xl p-0 overflow-hidden flex flex-col max-h-[85vh]">
        <DialogHeader className="p-6 pb-4 border-b border-border bg-background">
          <DialogTitle className="text-xl font-semibold text-foreground flex items-center gap-2">
            <Books className="w-7 h-7 text-primary" weight="fill" />{" "}
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex flex-col">
                    <span
                      className="font-medium text-foreground truncate w-48"
                      title={name}
                    >
                      {name}
                    </span>
                  ...
                  {activeCollection === name && (
                    <span className="text-[10px] uppercase font-semibold bg-primary text-primary-foreground px-2 py-0.5 rounded-full shadow-sm">
                      Actif
                    </span>
                  )}
                </div>
