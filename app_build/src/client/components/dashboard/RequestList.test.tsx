...
try { GlobalRegistrator.register(); } catch (e) { /* already registered */ }
    ...
    return {
      getVirtualItems: () => {
        return Array.from({ length: options.count }).map((_, i) => ({
          index: i, key: i, size: 28, start: i * 28
        }));
      },
...
describe('Component: RequestList', () => {
  ...
  it('renders correctly when no folders and requests', () => {
    render(<RequestList onOpenSettings={() => {}} onOpenCollections={() => {}} />);
  ...
  it('renders requests and folders correctly', () => {
    useStore.setState({
      folders: [],
      requests: [
        { id: 'req1', folderId: 'root', name: 'Get Users', method: 'GET', url: '/users', examples: [] }
      ]
    });
  ...
  it('renders starred requests and toggle folders', async () => {
    useStore.setState({
      ...
      requests: [
        { id: 'req1', folderId: 'f1', name: 'Get Users', method: 'GET', url: '/users', examples: [], isStarred: true, variants: [{ id: 'v1', name: 'Var1', isMocked: true } as any] },
        { id: 'req2', folderId: 'f1', name: 'Obsolete Req', method: 'POST', url: '/post', examples: [], isObsolete: true }
      ]
    });

    const { container } = render(<RequestList onOpenSettings={() => {}} onOpenCollections={() => {}} />);
